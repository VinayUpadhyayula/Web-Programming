...
function setupRoutes(app: Express.Application) {
  ...
  app.get(`${base}/:ss_name/:cell_id`, getHandler(app));
  app.get(`${base}/:ss_name`, getSheetHandler(app));
  app.put(`${base}/:ss_name`, loadSheetHandler(app));
  app.patch(`${base}/:ss_name/:cell_id*`, patchHandler(app));
  app.delete(`${base}/:ss_name/:cell_id`, deleteHandler(app));
  app.delete(`${base}/:ss_name`, clearHandler(app));
...
/****************** Handlers for Spreadsheet Cells and Complete Spreadsheets*********************/

function loadSheetHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      const result = await app.locals.ssServices.load(req.params.ss_name, req.body);
...
function getHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      const result = await app.locals.ssServices.query(req.params.ss_name, req.params.cell_id);
      if (!result.isOk) throw result;
      res.json(selfResult(req, result.val));
    }
    catch (err) {
...
function getSheetHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      const result = await app.locals.ssServices.dump(req.params.ss_name);
      if (!result.isOk) throw result;
      res.json(selfResult(req, result.val));
    }
    catch (err) {
...
function patchHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      let result;
      if (Object.keys(req.query).length === 0 || (req.query.expr && req.query.srcCellId)) {
        const errmsg = 'cannot call patch method with empty query params or both expression and source cellId'
        throw errResult(errmsg, { code: 'BAD_REQ' });
      }
      else if (req.query.expr) {
        result = await app.locals.ssServices.evaluate(req.params.ss_name, req.params.cell_id, req.query.expr);
      }
      else if (req.query.srcCellId) {
        result = await app.locals.ssServices.copy(req.params.ss_name, req.params.cell_id, req.query.srcCellId);
      }
      if (!result.isOk) throw result;
      res.json(selfResult(req, result.val));
    }
...
function deleteHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      const result = await app.locals.ssServices.remove(req.params.ss_name, req.params.cell_id);
      if (!result.isOk) throw result;
      res.json(selfResult(req, result.val));
    }
    catch (err) {
...
function clearHandler(app: Express.Application) {
  return async function (req: Express.Request, res: Express.Response) {
    try {
      const result = await app.locals.ssServices.clear(req.params.ss_name);
      if (!result.isOk) throw result;
      res.json(selfResult(req, result.val));
    }
    catch (err) {
